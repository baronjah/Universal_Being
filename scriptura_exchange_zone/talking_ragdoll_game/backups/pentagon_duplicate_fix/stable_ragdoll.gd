...
func _create_stable_ragdoll() -> void:
	...
	var head_mat = MaterialLibrary.get_material("default")
	head_mat.albedo_color = Color(0.9, 0.7, 0.6)
	head_mesh.material_override = head_mat
	FloodgateController.universal_add_child(head_mesh, head)
	...
	FloodgateController.universal_add_child(head_collision, head)
...
func _create_leg(is_left: bool) -> void:
	...
	FloodgateController.universal_add_child(leg_collision, leg)
