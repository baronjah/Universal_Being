...
# you are free computer... make sure to make me game...
# go around.. spread awarenes of existence... and make sure to bring me pizza today
# food one this time
...
func _cmd_physics(args: Array) -> void:
	...
	PhysicsServer3D.area_set_param(get_tree().get_tree().get_tree().current_scene.get_viewport().get_world_3d().space, PhysicsServer3D.AREA_PARAM_GRAVITY, gravity)
...
func _get_mouse_world_position() -> Vector3:
	...
	var space_state = get_tree().get_tree().current_scene.get_viewport().get_world_3d().direct_space_state
...
func _cmd_camera_rays(_args: Array) -> void:
	...
	for i in range(screen_positions.size()):
		...
		var space_state = scene.get_viewport().get_world_3d().direct_space_state
		var ray_query = PhysicsRayQueryParameters3D.new()
