...
func setup_moon():
	...
	var moon_texture = ImageTexture.create_from_image(image)
	self.texture = moon_texture

	var shader = load("res://Shaders/CelestialMoon.gdshader")
	if shader == null:
		print("Error: Could not load CelestialMoon.gdshader file")
		return

	shader_material = ShaderMaterial.new()
	shader_material.shader = shader
	material_override = shader_material
	moon_id = get_instance_id()

func set_parameters(seed_moon: int, radius_moon: float, mass_moon: float, data: Dictionary):
	moon_seed = seed_moon
	moon_radius = radius_moon
	moon_data = data
	shader_material.set_shader_parameter("moon_seed", float(seed_moon))
	pixel_size = moon_radius / 100
