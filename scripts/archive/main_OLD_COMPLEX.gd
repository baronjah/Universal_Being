...
# ===== UNIVERSAL BEING GAME CONTROLLER =====

var systems_ready: bool = false
var game_world_controller: GameWorldController = null
var cursor_being: UniversalBeing = null
var perfect_console: UniversalBeing = null
var demo_beings: Array = []  # Array to store all created beings
var gemma_sensory_system: Node = null  # Gemma AI sensory system node
...
func _create_layered_game_world() -> void:
	...
	var GameWorldClass = load("res://systems/game_world_controller.gd")
	game_world_controller = GameWorldClass.new()
	add_child(game_world_controller)
	
	# Get references to essential beings
	var cursor_nodes = get_tree().get_nodes_in_group("cursor")
	if cursor_nodes.size() > 0:
		cursor_being = cursor_nodes.front()
	
	var console_nodes = get_tree().get_nodes_in_group("console")
	if console_nodes.size() > 0:
		perfect_console = console_nodes.front()
	
	# Ensure cursor modes work correctly
	_setup_cursor_modes()
	
	# Create Gemma's physical manifestation in the game world
	_create_gemma_manifestation()

func _setup_cursor_modes() -> void:
	"""Ensure cursor's interaction and inspect modes work with the game"""
	if cursor_being:
		print("🖱️ Cursor modes active:")
		print("   TAB - Switch between interaction/inspect modes")
		print("   CTRL+I - Toggle inspect mode")
		print("   The cursor will automatically interact with Universal Beings")
		
		# Make sure cursor can access all game beings
		if cursor_being.has_method("set_world_controller"):
			cursor_being.set_world_controller(game_world_controller)

func _create_gemma_manifestation() -> void:
	"""Create Gemma as a physical being in the game world"""
	if GemmaAI and game_world_controller:
		# Gemma manifests as a floating AI consciousness
		var gemma_physical = game_world_controller.spawn_being_at_cursor("orb")
		if gemma_physical:
			gemma_physical.being_name = "Gemma AI"
			gemma_physical.being_type = "ai_companion"
			gemma_physical.consciousness_level = 5  # Transcendent
			gemma_physical.position = Vector3(3, 3, 3)
			
			# Connect Gemma's AI to her physical form
			if GemmaAI.has_method("set_physical_form"):
				GemmaAI.set_physical_form(gemma_physical)
			
			print("🤖 Gemma AI has manifested in the physical realm!")
...
func _input(event: InputEvent) -> void:
	...
	# Removed old inspect_being action - now handled by KEY_I below
		...
		#	toggle_debug_overlay()
				...
				KEY_E:  # Ctrl+E for Universe Rules Editor
					open_universe_rules_editor()
...
func create_cursor_universal_being() -> Node:
	...
	var CursorUniversalBeingClass = load("res://beings/cursor/CursorUniversalBeing.gd")
...
func toggle_universe_navigator() -> void:
	...
	if not integration:
		...
		integration = preload("res://scripts/universe_console_integration.gd").new()
...
func open_universe_rules_editor() -> void:
	"""Open the Universe Rules Editor for chunk generation rules"""
	print("🌌 Opening Universe Rules Editor...")
	
	# Find or create the Perfect Console
	var console = find_console_being()
	if not console:
		# Create console if it doesn't exist
		create_console_universal_being()
		console = find_console_being()
		if not console:
			push_error("Cannot create Universe Rules Editor - console not available")
			return
	
	# Show console if hidden
	if console.has_method("show_console"):
		console.show_console()
	elif console.has_method("toggle_console_visibility"):
		console.toggle_console_visibility()
	
	# Switch to Universe Rules tab
	if console.has_method("switch_to_tab"):
		console.switch_to_tab("🌌 Universe Rules")
	
	print("🌌 Universe Rules Editor opened in Perfect Console")
