...
@onready var consciousness_system = get_node("\1") as Node
@onready var ai_controller = get_node("\1") as Node
}
...
func _ready():
	...
	player_ship = current_system.get_node("\1") as Node
}
...
func _apply_consciousness_modifiers():
	...
	if player_ship:
		var ship_stats = player_ship.get_node("\1") as Node
		if ship_stats:
...
func connect_to_universe_simulation():
	var universe = get_node("\1") as Node
	if universe:
