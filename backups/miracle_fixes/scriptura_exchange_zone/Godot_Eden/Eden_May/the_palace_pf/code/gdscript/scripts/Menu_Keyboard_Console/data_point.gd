...
func set_connection_target(target_container, target_thing, target_datapoint):
	...
	var datapoint_path = main_node.scene_tree_jsh["main_root"]["branches"][target_container]["datapoint"]["datapoint_path"]
	var container_node = main_node.jsh_tree_get_node(target_container)
	
	if container_node:
		var thing_node = container_node.get_node_or_null(target_thing)
		if thing_node:
	...
	if scene_to_set:
		for scene_test in scene_to_set[1]:
			...
			if is_instance_valid(datapoint_things_dictionary[scene_test[1][0]]["node"]):
				...
				if datapoint_things_dictionary[scene_test[1][0]]["node"].position == new_position:
