...
#func check_all_things():
	...
	# If this is first check, store it
	if thread_stats.first_check == null:
		thread_stats.first_check = thread_pool.get_thread_stats()
	else:
		# Just update current state
		thread_stats.current_check = thread_pool.get_thread_stats()
		
	if thread_stats:
		#print(" thread_stats : " , thread_stats)
		store_thread_check(thread_stats)
	return true
