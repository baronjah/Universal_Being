...
func _setup_upright_controller() -> void:
	# Create walker as a child node
	var walker_script = load("res://scripts/core/simple_ragdoll_walker.gd")
	if walker_script:
		walker = Node3D.new()
		...
		# Walker will find body parts through parent
		print("🚶 [SevenPartRagdoll] Simple walker controller added")
	else:
		print("❌ [SevenPartRagdoll] Failed to load walker script!")
...
func _create_body_part(part_name: String) -> Node3D:
	...
	part.mass = 2.0  # 2kg per body part
	part.gravity_scale = 1.0
	part.linear_damp = 0.5  # Some damping to prevent wild movements
	part.angular_damp = 2.0  # More angular damping for stability
	...
	if randf() < 0.01:  # 1% chance per frame
		...
		if walker and walker.get("current_state") != null:
			match walker.current_state:
				walker.WalkState.STEPPING, walker.WalkState.BALANCING:
					state = "walking"
				walker.WalkState.FALLING:
					state = "falling"
				_:
					state = "idle"
