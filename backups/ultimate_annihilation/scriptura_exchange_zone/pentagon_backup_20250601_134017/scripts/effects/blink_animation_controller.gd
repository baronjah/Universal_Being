...
class_name BlinkAnimationController_blinkanimationcontroller
...
func trigger_blink(node_name: String = "", blink_count: int = 1) -> bool:
    ...
    if node_name.is_empty():
        ...
        for node_key in registered_nodes:
            _blink_node(node_key, blink_count)
        return true
...
func trigger_flicker(node_name: String = "", flicker_count: int = 3) -> bool:
    ...
    if node_name.is_empty():
        ...
        for node_key in registered_nodes:
            _flicker_node(node_key, flicker_count)
        return true
    elif registered_nodes.has(node_name):
        _flicker_node(node_name, flicker_count)
        return true
