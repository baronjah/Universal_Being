...
func _create_ragdoll_parts() -> void:
	...
	FloodgateController.universal_add_child(left_leg_mesh, left_leg)
	...
	FloodgateController.universal_add_child(left_leg_collision, left_leg)
	...
	FloodgateController.universal_add_child(right_leg_mesh, right_leg)
	
	var right_leg_collision = CollisionShape3D.new()
	right_leg_collision.shape = leg_col_shape
	right_leg_collision.position.y = -0.5
	FloodgateController.universal_add_child(right_leg_collision, right_leg)
	...
	FloodgateController.universal_add_child(foot_mesh, left_foot)
	...
	FloodgateController.universal_add_child(foot_collision, left_foot)
	...
	FloodgateController.universal_add_child(right_foot_mesh, right_foot)
	
	var right_foot_collision = CollisionShape3D.new()
	right_foot_collision.shape = foot_col_shape
	FloodgateController.universal_add_child(right_foot_collision, right_foot)
...
func _apply_materials() -> void:
	...
	body_material = MaterialLibrary.get_material("default")
	body_material.albedo_color = Color(0.8, 0.3, 0.3)
	body_material.roughness = 0.7
	
	# Leg material (darker red)
	leg_material = MaterialLibrary.get_material("default")
