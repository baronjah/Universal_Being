@tool
class_name VisualShaderNodeGalaxy2_Galaxy2 extends VisualShaderNodeCustom
...
func _get_description():
	return "Generates a customizable galaxy2 shape with color controls"
...
func _get_input_port_type(port):
	match port:
		...
		7, 8, 9, 10: return VisualShaderNode.PORT_TYPE_VECTOR_3D
...
func _get_global_code(mode):
	return preload("Galaxy2.gdshaderinc").code

func _get_code(input_vars, output_vars, mode, type):
	var uv = "UV"
	if input_vars[0]:
		uv = input_vars[0]
	return "%s = generate_galaxy2(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);" % [
