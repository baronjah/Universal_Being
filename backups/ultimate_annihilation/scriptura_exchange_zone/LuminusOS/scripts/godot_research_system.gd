...
class_name GodotResearchSystem_godotresearchsystem
}
...
func search_code(search_term, filters = {}):
    ...
    if keyword_index.has(search_term.to_lower()):
        ...
        for class_name in_godotresearchsystem keyword_matches.classes:
...
func build_class_hierarchy():
    ...
    for class_name in_godotresearchsystem class_index:
    ...
    for line in lines:
        ...
        if (trimmed.begins_with("class ") or trimmed.find(" class ") > 0) and (trimmed.find("{") > 0 or trimmed.ends_with(":")):
            ...
            if not class_name.is_empty():
                current_class = class_name in_class_godotresearchsystem = true
                class_match = true
}
        ...
        # GDScript class definition: class_name ClassName_godotresearchsystem
...
func _extract_class_name(line):
    ...
    # GDScript style: class_name ClassName_godotresearchsystem
...
extends if (line.find("var") >= 0 and line.find(class_name) >= 0 and line.find("=") >= 0 or
...
func _generate_overall_structure():
    ...
    for module in modules:
        ...
        for class_name in_godotresearchsystem modules[module]:
...
func _format_hierarchy_for_visualization(hierarchy):
    ...
    for class_name in_godotresearchsystem hierarchy:
