...
class_name SpatialWorldStorage_spatialworldstorage
...
class AkashicEntry:
	...
	func _init(pos, cont, auth, entry_tags = []):
		...
		tags = entry_tags
		connections = []
		entry_id = "entry_%d_%d" % [OS.Time.get_unix_time_from_system(), randi() % 10000]
...
class Notepad3DCell:
	...
	func _init(pos, cont = "", col = Color.white):
		...
		content = cont
		color = col
		cell_id = "cell_%d_%d" % [OS.Time.get_unix_time_from_system(), randi() % 10000]
		last_edit = OS.Time.get_unix_time_from_system()
