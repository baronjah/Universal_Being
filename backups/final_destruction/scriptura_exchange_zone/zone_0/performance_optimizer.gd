...
class_name PerformanceOptimizer_performanceoptimizer
...
func _find_node_by_class(node, class_name):
    if node.get_class() == class_name or_performanceoptimizer (node.get_script() and node.get_script().get_path().find(class_name.to_lower()) >= 0):
        return node
