...
class_name AutoAgentMode_autoagentmode
}
...
func _perform_turn_operations(turn):
    ...
    turn_operation.end_time = OS.Time.get_unix_time_from_system()
...
func _process_command(command, core_id):
    ...
    # Complete operation
    operation.end_time = OS.Time.get_unix_time_from_system()
...
# // Create exact duplicate
        ...
        word_trajectories[new_word].created = OS.Time.get_unix_time_from_system()
}
...
func get_auto_agent_status():
    return {
        ...
        "session_duration": OS.Time.get_unix_time_from_system() - session_start_time,
