...
class_name DimensionalColorSystem_dimensionalcolorsystem
extends \2
...
func colorize_line(line: String, base_frequency: int, symbol_boost: int = 10) -> String:
	...
	for i in range(line.length()):
		...
		match char:
			...
			"_":
				current_freq = min(MAX_FREQUENCY, current_freq + symbol_boost / 2.0)
			"@":
			...
			".":
				current_freq = max(MIN_FREQUENCY, current_freq - symbol_boost / 2.0)
			",":
				current_freq = max(MIN_FREQUENCY, current_freq - symbol_boost / 4.0)
			" ":
