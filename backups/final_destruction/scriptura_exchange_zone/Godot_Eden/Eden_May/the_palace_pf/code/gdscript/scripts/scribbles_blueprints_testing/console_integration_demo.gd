extends Node
class_name JSHConsoleIntegrationDemo_consoleintegrationdemo
...
# Command handlers
func _cmd_status(self, args = []) -> Dictionary:
    var detail_level = 1
    if args.size() > 0:
        detail_level = args[0]
...
func _cmd_snake(self, args = []) -> Dictionary:
...
func _cmd_shapes(self, args = []) -> Dictionary:
...
func _cmd_task(self, args = []) -> Dictionary:
...
func _cmd_visualize(self, args = []) -> Dictionary:
    var system = "all"
    if args.size() > 0:
