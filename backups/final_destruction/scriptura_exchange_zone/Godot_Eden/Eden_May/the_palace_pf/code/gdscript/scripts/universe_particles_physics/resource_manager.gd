extends Node
class_name ResourceManager_resourcemanager
}
...
# Initialize with universe-scale resources
static func initialize_universe_resources():
    var instance = get_instance()
    instance.set_scale("universe")
    return instance
}

func _init():
    if _instance:
        push_error("ResourceManager already exists. Use ResourceManager.get_instance() instead.")
    else:
        _instance = self
...
func unregister_from_physics(object: Node) -> bool:
    # Check if in active list
    var index = physics_objects.find(object)
    if index != -1:
        physics_objects.remove_at(index)
}

        # Process queued physics objects
        process_physics_queue()
        return true
}

    # Also check the queue
    for i in range(physics_queue.size()):
        if physics_queue[i].node == object:
            physics_queue.remove_at(i)
            return true
}
...
# Register an object for visibility
...
func clean_invalid_references(array: Array):
