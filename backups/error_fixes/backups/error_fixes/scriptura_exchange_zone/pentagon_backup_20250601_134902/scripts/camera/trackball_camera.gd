# 🏛️ Trackball Camera - Ragdoll physics and behavior system
...
func pentagon_ready() -> void:
	super.pentagon_ready()  # this allows overriding through inheritance
	ready()
...
func pentagon_input(event: InputEvent) -> void:
	super.pentagon_input(event)  # this allows overriding through inheritance
	input(event)
...
func pentagon_process(delta: float) -> void:
	super.pentagon_process(delta)  # this allows overriding through inheritance
	process(delta)
	process_roll(delta)
...
func process_actions(delta: float):
	if action_enabled:
		...
		if Input.is_action_pressed(action_up):
			add_inertia(Vector2(
				0.0,
				intent * Input.get_action_strength(action_up) * self.action_strength_y
			))
		if Input.is_action_pressed(action_down):
			add_inertia(Vector2(
				0.0,
				intent * Input.get_action_strength(action_down) * self.action_strength_y * -1.0
			))
		if Input.is_action_pressed(action_left):
			add_inertia(Vector2(
				intent * Input.get_action_strength(action_left) * self.action_strength_x,
				0.0
			))
