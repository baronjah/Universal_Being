...
func get_record_set(set_name: String) -> Dictionary:
	record_mutex.lock()
	if set_name in active_records:
		var record = active_records[set_name]
		record["last_accessed"] = Time.get_ticks_msec()
		record_mutex.unlock()
		return record["data"]
	elif set_name in cached_records:
		var cached = cached_records[set_name]
		# Move from cache to active
		active_records[set_name] = cached
		cached_records.erase(set_name)
		record_mutex.unlock()
		return cached["data"]
	record_mutex.unlock()
	return {}
...
func cache_record_set(set_name: String) -> bool:
	record_mutex.lock()
	if set_name in active_records:
		cached_records[set_name] = active_records[set_name]
		active_records.erase(set_name)
		record_mutex.unlock()
		return true
	record_mutex.unlock()
	return false
